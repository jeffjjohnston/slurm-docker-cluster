...
def jsonHeader(task, sample_id) {
    ...
    """\
    printf '{"workflow":"%s","nextflow_run":"%s","process":"%s","sample":"%s","attempt":"%s","user":"%s","slurm_job_id":"%s","slurm_node":"%s"}\\n' \
...
/*
 * PROCESS 3: coverage_qc  (uses BAM only)
 */
process COVERAGE_QC {
...
/*
 * PROCESS 4: annotation (uses VCF only)
 */
...
/*
 * PROCESS 5: db_submission (needs BAM + VCF)
 */
